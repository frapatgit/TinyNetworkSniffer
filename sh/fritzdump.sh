...
if [ -z "$FRITZPWD" ] || [ -z "$FRITZUSER" ]  ; then echo "$TAG Username/Password empty. Usage: $0 <username> <password>" ; exit 1; fi

echo "$TAGTrying to login into $FRITZIP as user $FRITZUSER"
...
if [[ $SID =~ ^0+$ ]] ; then echo "$TAG Login failed. Did you create & use explicit Fritz!Box users?" ; exit 1 ; fi
...
echo "$TAG Capturing traffic on Fritz!Box interface $IFACE ..." 1>&2

while true; do
  echo "$TAG Capturing traffic on Fritz!Box interface $IFACE for 10 s ..."
  ...
  if [[ -n $(ps -e | awk '{print $1}' | grep "$wget_pid") ]]; then
    ...
    sleep 1
    echo "$TAG process capture terminated after 10 seconds ----- 2"
  fi
done